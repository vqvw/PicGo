...
func (c *Client) readPump() {
	...
	for {
		...
		// unmarshal clientMessage
		var clientMessage Message
		err = json.Unmarshal(message, &clientMessage)
		check(err)

		// add this client to clientMessage so the
		// hub knows who received this message
...
func serveWs(hub *Hub, w http.ResponseWriter, r *http.Request) {
	...
	if len(hub.clients) >= 10 {
		client.send <- Message{Type: "full"}
